...
import { getTravels, deleteTravel } from '../api/travels-api'
...
export class Travels extends React.PureComponent<TravelsProps, TravelsState> {
  ...
  onDeleteTravel = async(travelId: string) => {
    try {
          ...
          {this.state.travels.map((travel, pos) => {
            return (
              <Table.Row key={travel.id}>
                ...
                <Table.Cell width={2}>
                  <Button onClick={() => {this.setState({loadingTravels: true}); this.onDeleteTravel(travel.id);}} icon color="blue">
                      <Icon name="delete"/>
                  </Button>
                </Table.Cell>
              </Table.Row>
